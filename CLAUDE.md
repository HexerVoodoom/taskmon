...
| 💠 Bits | Moeda dos minijogos (`gamePoints`), fonte de calculadora mono (`utils/currency.ts`; elemento precisa estar sob `.tk-keep-mono` p/ manter a fonte); texto preto ou branco conforme contraste do fundo em cada local (sem cor fixa no `bitsStyle`). Dino floor(score/100) · PPT 5/vitória · Masmorra Bits/inimigo + bônus `10+5×(andar−1)`. **Sem drops de itens de evolução** (removidos). |
