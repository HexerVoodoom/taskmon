...
| 🧺 Piquenique | Meta COOPERATIVA semanal na 4ª casinha (`TogetherHome`): soma das tarefas reais dos 3 perfis na semana (leitura pura de `completedTasks`, `FAMILY_WEEK_GOAL=15`); bater a meta renderiza o piquenique dos 3 pets. Medidor mostra a contribuição dos **3** pets sempre (perfil sem save conta 0 e continua aparecendo, com o pet canônico de `PET_TYPES[i]`), ordem fixa e pesos iguais. NUNCA ranking entre perfis. |
| 🎀 Presentes | Na 4ª casinha: transfere 1 comida do inventário do perfil ATIVO pro inventário de outro perfil (`handleGiftFood` no App decrementa o ativo via setGameState e escreve o alvo direto no localStorage — alvo é sempre inativo). Só perfis COM save podem receber. 💗/🌀 não são presenteáveis. |
