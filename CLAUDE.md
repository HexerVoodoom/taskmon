...
| 🧺 Piquenique | Meta COOPERATIVA semanal na 4ª casinha (`TogetherHome`): soma das tarefas reais dos 3 perfis na semana (leitura pura de `completedTasks`, `FAMILY_WEEK_GOAL=15`); bater a meta renderiza o piquenique dos 3 pets. Medidor mostra a contribuição de cada pet (ordem fixa, pesos iguais). NUNCA ranking entre perfis. |
| 🎀 Presentes | **DESATIVADO** (UI removida da 4ª casinha a pedido). Lógica intacta pra religar: `handleGiftFood` no App (decrementa o ativo via setGameState, escreve o alvo — sempre inativo — direto no localStorage) + `sendGift`/`giftableFoods` no `TogetherHome`; falta só restaurar a barra de botões antes da dica de carinho do rodapé. |
