...
| ⚔️ Masmorra | `utils/dungeon.ts`: run = 5 andares × 6 inimigos = **sombras dos pets** (`shadow-<pet>-<fase>`, 2 por fase, fase 1→3, `LADDER_TIERS`). Dificuldade andar F = base+(F−1); base persiste e reseta SEMANALMENTE. Perder = −1 coração real (não entra com ≤1). Drop 💗 5%/inimigo máx. 2/dia. Concluir 5 andares → 🌀 Glitchtama + base+1. Cenários retrô por andar (`utils/dungeonScenes.ts`). `PLAYER_STATS` por fase no DungeonGame. |
| 🛒 Loja | 3 ABAS (Itens/Cenários/Missões, `ShopModal` 8-bit). Catálogo `utils/shop.ts`: **Coraçãozinho** (150) + 11 cenários (150–250) + 6 cenários de missão (300, `unlock: mission`). Glitchtama NUNCA é vendido. **Missões** (`utils/missions.ts`): fase 2, fase 3, 100 kills, 3 runs, Dino 1000, 30 dias perfeitos totais. |
