...
| ❤️ Corações (HP) | Perde na virada do dia: `floor((1 − feitas/meta) × maxHP)`, meta = `min(cadastradas, requisito da fase)`. HP aceita 0.5. **Dia sem tarefa cadastrada é neutro** (não perde HP nem dia perfeito). HP 0 → **modo dorminhoco** (`sleepyMode`): mantém a fase, dorme com 0.5 HP; curar até ≥1 acorda. Regressão de fase só MANUAL (botão "Voltar" da Evolução). |
| 🛡️ Escudo | 1×/semana (semana = segunda, `weekKey` em `utils/economy.ts`), automático na virada: um dia ruim não perde coração nem dias perfeitos (`shieldUsedWeek`). Não é consumido em dia perfeito/neutro. |
| 🧺 Piquenique | **Cesta compartilhada** (`utils/familyBasket.ts`, chave `digiapp-family-basket` NÃO escopada por perfil): toda tarefa real de qualquer perfil deposita uma **cópia** da comidinha na cesta (extra — ninguém perde do próprio inventário). Meta `FAMILY_BASKET_GOAL=15`; zera na virada da semana (`weekKey`). Cheia → piquenique renderiza na 4ª casinha e **cada perfil pega 1 💗 uma vez** (`claimed[]`, botão "Pegar meu presente"). Medidor mostra os **3** pets sempre (perfil sem save conta 0, pet canônico de `PET_TYPES[i]`), ordem fixa e pesos iguais. NUNCA ranking. |
| 🎀 Doar item | Na **mochila** (`ItemsWindow`): tocar um item abre **Usar × Doar** — doar escolhe o pet de outra casinha e transfere 1 unidade (`handleGiftFood` no App decrementa o ativo via setGameState e escreve o alvo direto no localStorage — alvo é sempre inativo). Só perfis COM save aparecem como alvo. |
| 🗣️ Elogio | Conclusão de tarefa fala + mostra elogio de PROCESSO (`celebrateTask` no App, frases rotativas PT/EN) — nunca elogio de atributo ("inteligente"), sempre esforço/estratégia. TTS respeita o mute. |
| 🎓 Educativos | `FeirinhaGame` (contar frutas / pagar com moedas 1-5-10, fregueses = os outros 2 pets) e `LetterHuntGame` (achar letra / completar palavra), ambos com narração TTS e 2 modos por idade. +2 Bits/acerto via `onEarnPoints` (contam no teto diário). |
...
| ⭐ Dia perfeito | `tarefas ≥ min(cadastradas, requisito) && ≥1 cadastrada && energia cheia` → +1 perfectDay. Dia não-perfeito → −1 (exceto dia neutro sem tarefa e dia protegido pelo escudo). **🌀 Glitchtama** (recompensa dos 5 andares da masmorra) dá +1 ao usar. |
...
| 💠 Bits | Moeda (`gamePoints`), fonte de calculadora mono (`utils/currency.ts`; elemento precisa estar sob `.tk-keep-mono` p/ manter a fonte); texto preto ou branco conforme contraste do fundo em cada local (sem cor fixa no `bitsStyle`). **Tarefa/atividade real concluída → +10 Bits na hora** (`TASK_BITS`, `utils/economy.ts`). Minijogos: Dino floor(score/100) · PPT 5/vitória · Masmorra Bits/inimigo + bônus `10+5×(andar−1)`, mas com **teto diário** `20 + 30×(tarefas reais do dia)` (`gameBitsCapFor`, contador `gameBitsToday` reseta na virada). **Sem drops de itens de evolução** (removidos). |
...
| 🐺 Atropela Lobisomem | `WerewolfRunGame`: corrida noturna com os 3 pets no carro, **5 faixas** (estrada larga — ficar parado no centro não pega quase ninguém). Lobisomem atropelado = +1 ponto; **cervo 🦌 atropelado = −3 pontos** (flash vermelho + `-3` na tela), placar nunca abaixo de 0. Os Bits são creditados **no fim da rodada** com o placar final (pra penalidade do cervo custar Bits de verdade). Sem custo de coração. Sprite do cervo: `src/assets/werewolf/deer.png` (placeholder pixel-art gerado — trocar pelo definitivo). |
| 🛒 Loja | Aba 🍽️ Comida: 11 tipos (`utils/shop.ts` `SHOP_ITEMS`, 10–22 Bits) — soma no `foodInventory`, +1 energia. Aba 🎁 **Prêmios de verdade**: catálogo de recompensas do mundo real (`realRewards`, default em `utils/economy.ts`), editável pelo responsável via portão de adulto (continha de ×); resgate desconta Bits, grava em `rewardRedemptions` (máx. 50) e mostra um cartão/voucher. Coraçãozinho/Glitchtama NÃO são vendidos (só drop da masmorra, `SPECIAL_ITEMS`). Sistema de missões e cenários compráveis foi removido (cenários já possuídos por saves antigos continuam equipáveis via `equippedBackground`, só não há mais UI pra trocar). |
